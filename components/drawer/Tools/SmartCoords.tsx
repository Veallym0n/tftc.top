...
const SmartCoords = NiceModal.create(({ externalTarget }: SmartCoordsProps) => {
  ...
  const { addTempPin } = useCacheStore();
  ...
  const handleLocateOnMap = () => {
    ...
    useMapStore.getState().flyTo(coords.lat, coords.lng);
  ...
  return (
    <Modal
      ...
      title={externalTarget ? targetName : '智能坐标'}
      footer={null}
    >
      <div className="space-y-4">
        ...
        {!externalTarget && (
            <div>
              <label className="block text-xs font-black text-slate-400 uppercase mb-1">粘贴坐标</label>
              <input
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="如: 31.2304, 121.4737"
        ...
        {coords ? (
          ...
          <div className="flex flex-col items-center justify-center py-6 text-slate-300 border-2 border-dashed border-slate-300 rounded-xl bg-slate-50">
             ...
             <div className="text-xs font-bold">等待输入坐标...</div>
          </div>
        )}
      </div>
    </Modal>
